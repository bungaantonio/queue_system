...
def get_user_queue_status_by_identity(
    ...
    user = (
        user_crud.get_user_by_id_number(db, id_number)
        if id_number
        else user_crud.get_user_by_phone(db, phone)
    )
    if not user:
        raise QueueException("user_not_found")

    queue_item = consult.get_existing_queue_item(db, user.id)
    if not queue_item:
...
def get_user_queue_status(db: Session, user_id: int) -> QueueDetailItem:
    ...
    item = consult.get_by_user(db, user_id)
    if not item:
...
def get_active_user(db: Session) -> Optional[QueueDetailItem]:
    ...
    item = consult.get_active_service_item(db)
    if not item:
...
def get_next_user_to_call(db: Session) -> Optional[QueueDetailItem]:
    ...
    item = consult.get_next_waiting_item(db)
    if not item:
        return None
    return QueueDetailItem.from_orm_item(item)
...
def list_waiting_and_called_items(db: Session) -> List[QueueListItem]:
    ...
    waiting_items = consult.get_all_waiting(db)
    pending_item = consult.get_pending_verification_item(db)
...
def get_pending_verification_user(db: Session) -> QueueDetailItem:
    ...
    item = consult.get_pending_verification_item(db)
    if not item:
