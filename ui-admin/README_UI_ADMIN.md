...
```bash
docker-compose up --build ui-admin-dev
```

* Usa `Dockerfile.ui-admin.dev`
* Monta código local como volume para hot reload
* UI Admin disponível em <http://localhost:3002>
...
```bash
docker-compose up --build ui-admin
```

* Usa `Dockerfile.ui-admin` (Nginx + build otimizado)
* UI Admin disponível em <http://localhost:3002>
