...
export const SLAMetricsChart = () => {
  const slaMetrics: SLAMetric[] = [
    ...
    {
      label: "Resolução ao Primeiro Contacto",
      current: 91.5,
      target: 90,
      status: "success",
    },
      ...
      >
        ...
        >
          <Box sx={{ position: "relative", display: "inline-flex" }}>
            <CircularProgress
              ...
              sx={{
                color:
                  overallSLA >= targetSLA ? "success.main" : "warning.main",
              }}
            />
            <Box
              sx={{
        ...
        <Box>
          ...
          <Typography variant="body2">
            ...
            >
              {slaMetrics.filter((m) => m.status === "success").length} de{" "}
              {slaMetrics.length} métricas
            </Typography>
          </Typography>
        </Box>
      </CardContent>
